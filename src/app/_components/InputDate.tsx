...
interface DateInputProps
  ...
  excludeIntervals?: { start: Date; end: Date }[];
}
...
const InputDate: FC<DateInputProps> = ({
  ...
  return (
    <div
      className={`focus-within:border-netral-900 relative flex w-full items-center gap-2 rounded-lg border-2 px-4 py-2 ${
        disabled ? "bg-netral-200 cursor-not-allowed" : ""
      } border-netral-400`}
    >
      <div className="flex-shrink-0">
        <svg
          className={`h-6 w-6 text-blue-600 $${
            disabled ? "text-netral-600" : ""
          }`}
          viewBox="0 0 24 24"
          fill="none"
